...
"""
    assert response.files[1].contents.decode("utf-8") == expected_enums
...
def test_copy_query_generation():
    """Test that :copy queries generate proper COPY functionality."""
    # Create test queries with :copy annotation
    queries = [
        Query(
            name="bulk_insert_users",
            text="COPY users (name, email) FROM STDIN",
            cmd=":copy",
            params=[],
        ),
    ]
...
BULK_INSERT_USERS = r"""-- name: bulk_insert_users \\:copy
COPY users (name, email) FROM STDIN
"""

async def query_bulk_insert_users(conn: DBConn, data: AsyncIterator[tuple[Any, ...]]) -> None:
