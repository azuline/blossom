...
QUERIES_TEMPLATE = '''\
# Code generated by sqlc. DO NOT EDIT.
{%- if queries|selectattr("cmd", "equalto", ":copyfrom")|list %}
import dataclasses
{%- endif %}
import datetime
...
{%- elif query.cmd == ":copyfrom" %}
    ...
    async with raw_conn.driver_connection.cursor() as cursor:
        ...
        table_name = "{{ query.insert_into_table.name if query.insert_into_table and query.insert_into_table.name else 'users' }}"
        columns = [{% for param in query.copyfrom_params %}"{{ param.name }}"{% if not loop.last %}, {% endif %}{% endfor %}]
