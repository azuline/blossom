...
# nosemgrep
TEMPLATE = """\
/* Code generated by backend/script/codegen/rpc.py. DO NOT EDIT. */
...
def codegen_rpc_models(router: RPCRouter, dst: Path) -> None:
    ...
    with dst.open("w") as fptr:
        fptr.write(code)
    os.chdir(MONOREPO_ROOT / "frontend")
...
def codegen_product() -> None:
    router = create_router_product()
    # TODO: god mode
    # Add the hidden routes.
    # router2 = create_product_god_mode_routes()
    # for r in router2.routes:
    #     router.add_route(r, flags=["god_mode"])
    codegen_rpc_models(router, MONOREPO_ROOT / "frontend/product/src/z-rpc/types.ts")


def codegen_panopticon() -> None:
    codegen_rpc_models(create_router_panopticon(), MONOREPO_ROOT / "frontend/panopticon/src/z-rpc/types.ts")
