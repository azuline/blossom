...
# python -m grpc_tools.protoc -I tools/codegen_db_plugin --python_betterproto_out=tools/codegen_db_plugin
...
{%- elif query.cmd == ":batchone" %}
    ...
    async with raw_conn.driver_connection.cursor() as cursor:
        await cursor.executemany({{ query.constant_name }}, [{{ query.batch_param_dict }} for batch_item in batch_data], returning=True)
...
def generate_queries(queries: list[Query], catalog: Catalog | None = None) -> str:
    ...
    def process_query_text(query: Query) -> str:
        ...
        if query.cmd == ":copyfrom":
            # For COPY FROM operations, generate the COPY statement
            param_names = [param.column.name for param in query.params]
