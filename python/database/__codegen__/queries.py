...
class AsyncQuerier:
    ...
    async def authn_linked_organization_fetch(self, *, user_id: str, id: str) -> Optional[models.Organization]:
        row = (await self._conn.execute(sqlalchemy.text(AUTHN_LINKED_ORGANIZATION_FETCH), {"p1": user_id, "p2": id})).first()
        if row is None:
            return None
    ...
    async def authn_most_recently_accessed_organization_fetch(self, *, user_id: str) -> Optional[models.Organization]:
        row = (await self._conn.execute(sqlalchemy.text(AUTHN_MOST_RECENTLY_ACCESSED_ORGANIZATION_FETCH), {"p1": user_id})).first()
        if row is None:
            return None
        return models.Organization(
            id=row[0],
    ...
    async def authn_session_create(self, *, user_id: str, organization_id: Optional[str]) -> Optional[models.Session]:
        row = (await self._conn.execute(sqlalchemy.text(AUTHN_SESSION_CREATE), {"p1": user_id, "p2": organization_id})).first()
        if row is None:
            return None
    ...
    async def authn_session_expire(self, *, id: str) -> None:
        await self._conn.execute(sqlalchemy.text(AUTHN_SESSION_EXPIRE), {"p1": id})

    async def authn_session_fetch_by_user(self, *, user_id: str) -> Optional[models.Session]:
        row = (await self._conn.execute(sqlalchemy.text(AUTHN_SESSION_FETCH_BY_USER), {"p1": user_id})).first()
        if row is None:
            return None
        return models.Session(
            id=row[0],
    ...
    async def authn_user_fetch_by_email(self, *, email: str) -> Optional[models.User]:
        row = (await self._conn.execute(sqlalchemy.text(AUTHN_USER_FETCH_BY_EMAIL), {"p1": email})).first()
        if row is None:
            return None
        return models.User(
            id=row[0],
    ...
    async def organization_add_user(self, *, organization_id: str, user_id: str) -> Optional[models.OrganizationsUser]:
        row = (await self._conn.execute(sqlalchemy.text(ORGANIZATION_ADD_USER), {"p1": organization_id, "p2": user_id})).first()
        if row is None:
            return None
    ...
    async def organization_create(self, *, name: str, inbound_source: str) -> Optional[models.Organization]:
        row = (await self._conn.execute(sqlalchemy.text(ORGANIZATION_CREATE), {"p1": name, "p2": inbound_source})).first()
        if row is None:
            return None
        return models.Organization(
            id=row[0],
    ...
    async def organization_fetch(self, *, id: str) -> Optional[models.Organization]:
        row = (await self._conn.execute(sqlalchemy.text(ORGANIZATION_FETCH), {"p1": id})).first()
        if row is None:
            return None
        return models.Organization(
            id=row[0],
    ...
    async def organization_fetch_all(self, *, user_id: str) -> AsyncIterator[models.Organization]:
        result = await self._conn.stream(sqlalchemy.text(ORGANIZATION_FETCH_ALL), {"p1": user_id})
    ...
    async def pipeline_organization_id_fetch_all(self) -> AsyncIterator[str]:
        result = await self._conn.stream(sqlalchemy.text(PIPELINE_ORGANIZATION_ID_FETCH_ALL))
        async for row in result:
            yield row[0]

    async def rpc_unexpired_session_fetch(self, *, id: str) -> Optional[models.Session]:
        row = (await self._conn.execute(sqlalchemy.text(RPC_UNEXPIRED_SESSION_FETCH), {"p1": id})).first()
        if row is None:
            return None
        return models.Session(
            id=row[0],
    ...
    async def test_organization_create(self, *, name: str, inbound_source: str) -> Optional[models.Organization]:
        row = (await self._conn.execute(sqlalchemy.text(TEST_ORGANIZATION_CREATE), {"p1": name, "p2": inbound_source})).first()
        if row is None:
            return None
        return models.Organization(
            id=row[0],
    ...
    async def test_organization_user_create(self, *, user_id: str, organization_id: str) -> Optional[models.OrganizationsUser]:
        row = (await self._conn.execute(sqlalchemy.text(TEST_ORGANIZATION_USER_CREATE), {"p1": user_id, "p2": organization_id})).first()
        if row is None:
            return None
        return models.OrganizationsUser(
            id=row[0],
    ...
    async def test_session_create(self, *, user_id: str, organization_id: Optional[str], expired_at: Optional[datetime.datetime], last_seen_at: datetime.datetime) -> Optional[models.Session]:
        row = (await self._conn.execute(sqlalchemy.text(TEST_SESSION_CREATE), {
            "p1": user_id,
            "p2": organization_id,
            "p3": expired_at,
            "p4": last_seen_at,
    ...
    async def test_user_create(self, *, name: str, email: str, password_hash: Optional[str], signup_step: str, is_enabled: bool) -> Optional[models.User]:
        row = (await self._conn.execute(sqlalchemy.text(TEST_USER_CREATE), {
            "p1": name,
            "p2": email,
            "p3": password_hash,
            "p4": signup_step,
    ...
    async def user_create(self, *, name: str, email: str, password_hash: Optional[str], signup_step: str) -> Optional[models.User]:
        row = (await self._conn.execute(sqlalchemy.text(USER_CREATE), {
            "p1": name,
            "p2": email,
            "p3": password_hash,
            "p4": signup_step,
    ...
    async def user_fetch(self, *, id: str) -> Optional[models.User]:
        row = (await self._conn.execute(sqlalchemy.text(USER_FETCH), {"p1": id})).first()
        if row is None:
            return None
        return models.User(
            id=row[0],
    ...
    async def vault_secret_create(self, *, organization_id: str, ciphertext: str) -> Optional[models.VaultedSecret]:
        row = (await self._conn.execute(sqlalchemy.text(VAULT_SECRET_CREATE), {"p1": organization_id, "p2": ciphertext})).first()
        if row is None:
            return None
    ...
    async def vault_secret_delete(self, *, id: str) -> None:
        await self._conn.execute(sqlalchemy.text(VAULT_SECRET_DELETE), {"p1": id})

    async def vault_secret_fetch(self, *, id: str) -> Optional[models.VaultedSecret]:
        row = (await self._conn.execute(sqlalchemy.text(VAULT_SECRET_FETCH), {"p1": id})).first()
        if row is None:
            return None
        return models.VaultedSecret(
            id=row[0],
