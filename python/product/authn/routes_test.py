...
async def test_auth_login_success(t: ProductFixture) -> None:
    ...
    async with (await t.rpc.underlying_client()).session_transaction() as quart_sess:
...
async def test_auth_login_success_impermanent(t: ProductFixture) -> None:
    user = await t.factory.user()
    resp = await t.rpc.call(login, LoginIn(email=user.email, password="password", permanent=False, organization_id=None))
    await t.rpc.assert_success(resp)
    async with (await t.rpc.underlying_client()).session_transaction() as quart_sess:
...
async def test_auth_login_nonexistent_organization_id(t: ProductFixture) -> None:
    user = await t.factory.user()
    resp = await t.rpc.call(login, LoginIn(email=user.email, password="password", permanent=True, organization_id="garbage"))
    await t.rpc.parse_error(resp, AuthOrganizationNotFoundError)
...
async def test_auth_login_nonexistent_email(t: ProductFixture) -> None:
    resp = await t.rpc.call(login, LoginIn(email=generate_email(), password="password", permanent=True, organization_id=None))
    await t.rpc.parse_error(resp, InvalidCredentialsError)
...
async def test_auth_login_wrong_password(t: ProductFixture) -> None:
    user = await t.factory.user()
    resp = await t.rpc.call(login, LoginIn(email=user.email, password="badpassword", permanent=True, organization_id=None))
    await t.rpc.parse_error(resp, InvalidCredentialsError)
...
async def test_auth_logout(t: ProductFixture) -> None:
    ...
    resp = await t.rpc.call(logout, None)
    await t.rpc.assert_success(resp)
