...
async def query_organization_fetch(conn: DBConn, *, id: str) -> models.OrganizationModel:
    ...
    if row is None:
        raise NotFoundError(resource="organization_fetch", key_name="id", key_value=str(id))
...
async def query_organization_create(conn: DBConn, *, name: str, inbound_source: str) -> models.OrganizationModel:
    ...
    if row is None:
        raise NotFoundError(resource="organization_create", key_name="name", key_value=str(name))
    return models.OrganizationModel(
        id=row[0],
        created_at=row[1],
        updated_at=row[2],
...
async def query_organization_user_add(conn: DBConn, *, organization_id: str, user_id: str) -> models.OrganizationsUserModel:
    ...
    if row is None:
        raise NotFoundError(resource="organization_user_add", key_name="organization_id", key_value=str(organization_id))
