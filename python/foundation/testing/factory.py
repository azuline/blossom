...
from foundation.testing.__codegen_db__.queries import query_test_organization_create, query_test_organization_user_create, query_test_session_create, query_test_user_create
...
class TestFactory:
    ...
    @staticmethod
    async def organization(*, name: str | None = None, inbound_source: OrganizationsInboundSourceEnum = "organic") -> OrganizationModel:
        name = name or generate_name()
        async with xact_admin() as conn:
            return await query_test_organization_create(conn, name=name, inbound_source=inbound_source)
    ...
    async def user(
        *,
        ...
        async with xact_admin() as conn:
            return await query_test_user_create(
                conn,
                name=name or generate_name(),
    ...
    @staticmethod
    async def customer() -> tuple[UserModel, OrganizationModel]:
    ...
    @staticmethod
    async def organization_user_create(*, user_id: str, organization_id: str) -> OrganizationsUserModel:
        async with xact_admin() as conn:
            return await query_test_organization_user_create(conn, user_id=user_id, organization_id=organization_id)
    ...
    async def session(
        *,
        ...
        async with xact_admin() as conn:
            return await query_test_session_create(
                conn,
                user_id=user.id,
